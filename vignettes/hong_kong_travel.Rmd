...
results <- dag$run(initial_state = wf$initial_state, max_steps = 15, checkpointer = checkpointer)
...
audit_res <- results$state$get("Validator")
if (!is.null(audit_res$report)) {
  cat(audit_res$report)
} else {
