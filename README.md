...
1.  **🔍 Auditability**: It maintains a central memory system that continuously saves workflow checkpoints (using a DuckDB database), ensuring a complete, verifiable record of all AI interactions and data changes.
2.  **🛡️ Filesystem Safety**: By leveraging **Git worktrees**, `HydraR` creates temporary, isolated project branches for each agent working in parallel. This prevents file corruption and race conditions when multiple tasks run simultaneously.
3.  **🏗️ Portable Workflows**: By defining agent logic using standard **YAML** files and **Mermaid.js** diagrams, `HydraR` separates the workflow's design from the code that runs it. This text-based blueprint acts as an "OpenAPI for Agents"—a shareable format that is easy to audit and version-control.
...
- **Git Worktree**: A specialized Git feature that allows `HydraR` to create a separate, temporary version of your project for an agent to work in, preventing them from overwriting your main files accidentally.
...
- **[ellmer](https://github.com/tidyverse/ellmer)**: Excellent for high-level conversational/chat interfaces. `HydraR` can use `ellmer` as a backend driver for individual agents within a larger managed graph.
- **[mall](https://github.com/simonpcouch/mall)**: Provides a concise syntax for "mapping" LLM calls over data. `HydraR` is designed for more complex research pipelines that require memory, branching logic, and error recovery.
- **[gptstudio](https://github.com/MichelNivard/gptstudio)**: Tools for IDE-centric coding assistance. `HydraR` is built for **reproducible, non-interactive pipelines** and automation that can run independently of the RStudio IDE.
- **[LangGraph / CrewAI (Python)](https://github.com/langchain-ai/langgraph)**: These are the Python equivalents. `HydraR` provides a native R alternative that integrates directly with R's statistical and visualization ecosystems without the "translation cost" of using `reticulate` for core logic.
...
- **🖥️ CLI-First Drivers**: High-fidelity drivers for local and provider-based CLIs (Gemini, Claude, OpenAI), ensuring tool calls are robust.
...
We recommend using the [`DiagrammeR`](https://dgritree.github.io/DiagrammeR/) package to render your DAGs:
...
- **Local LLMs**: Integration with specialized local CLI wrappers (e.g., Ollama).
