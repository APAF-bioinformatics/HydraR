...
Researchers can use \texttt{HydraR} to design workflows as step-by-step pipelines (Directed Acyclic Graphs) or looping state machines, where each step can be an AI-driven task, a predictable R function, or an automated data auditor. \texttt{HydraR} connects seamlessly to multiple AI models, supporting both popular cloud providers (Gemini, Claude, OpenAI, GitHub Copilot) and local deployments via Ollama. Users outline their workflows using standard \texttt{YAML} configuration files that integrate \texttt{Mermaid.js} diagrams. This allows researchers to visually chart their processes and define reactive pathways (e.g., ``if a task fails, route back to Node B''). To ensure the workflow executes properly, a built-in validation engine rigorously inspects the configuration before running---checking for syntax errors and structural flaws. Although \texttt{HydraR} allows workflows to loop back on themselves to enable agents to self-correct mistakes, it prevents infinite loops by enforcing a customizable execution ``fuse'' (which halts the process after a set number of steps) alongside continuous data checkpointing.

\section{Statement of Need}\label{statement-of-need}

The contemporary R ecosystem lacks a unified framework that provides both high-level coordination for AI agents and the safe management of local files. While existing packages, such as \texttt{ellmer} (\citeproc{ref-ellmer}{Wickham et al., 2025}), focus primarily on conversational interfaces and \texttt{mall} (\citeproc{ref-mall}{Couch, 2024}) excels at applying LLM operations to structured data, neither is equipped to securely save the workflow's progress (its ``state'') or provide the isolated workspaces needed for long-running, multi-agent collaborations.

\texttt{HydraR} addresses three fundamental challenges in this domain:
\begin{enumerate}
    \item \textbf{Auditability}: It maintains a central memory system that continuously saves workflow checkpoints (using the DuckDB database \citeproc{ref-duckdb}{Raasveldt \& Mühleisen, 2019}), ensuring a complete, verifiable record of all AI interactions and data changes.
    \item \textbf{Filesystem Safety}: By leveraging Git worktrees, \texttt{HydraR} creates temporary, isolated project branches for each agent working in parallel. This prevents file corruption and race conditions when multiple tasks run simultaneously.
    \item \textbf{Portable Workflows}: By defining agent logic using standard \texttt{YAML} files and \texttt{Mermaid.js} diagrams, \texttt{HydraR} separates the workflow's design from the code that runs it. This text-based blueprint acts like an ``OpenAPI for Agents''---a shareable format that is easy to audit in R and can be exported to other AI development frameworks like LangGraph or Google's Agent Development Toolkit.
\end{enumerate}
...
While tools like \texttt{reticulate} (\citeproc{ref-reticulate}{Ushey et al., 2024}) allow researchers to connect R with popular Python-based AI frameworks (such as LangChain), this approach often creates compatibility issues when translating complex statistical data structures between the two programming languages. \texttt{HydraR} avoids this translation barrier by being built entirely using R's native architectural frameworks (\citeproc{ref-R6}{Chang, 2021}). This native design ensures that scientific data remains intact and allows researchers to use their familiar error-checking tools. Furthermore, existing AI tools in R generally serve different purposes: \texttt{ellmer} (\citeproc{ref-ellmer}{Wickham et al., 2025}) focuses on interactive chat features rather than automated workflows, while \texttt{gptstudio} (\citeproc{ref-gptstudio}{Nivard \& Wade, 2024}) functions as an interactive coding assistant. In contrast, \texttt{HydraR} is specifically designed to manage long-running, autonomous research tasks where multiple AI agents work in parallel and securely save their progress.
...
* \textbf{Node implementation details}: The \texttt{AgentLLMNode} and \texttt{AgentLogicNode} \texttt{R6} classes encapsulate LLM-driven prompts and deterministic pure-R logic steps, respectively. They utilize pluggable \texttt{AgentDriver} backends for provider-agnostic model communication.
