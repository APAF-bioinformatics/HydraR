...
- **Paper Synchronization**: Aligned README descriptions and examples with the content of the ArXiv/JOSS manuscript.
- **High-Resolution Assets**: Migrated high-quality figures and workflow diagrams from the paper submission to the package's documentation assets (`man/figures/`).
- **Standardized Technical Sources**: Each markdown vignette now provides explicit links back to its source `.Rmd` counterpart for Reproducible Research.
